...
export function LiveVisitCard({ v }: { v: Visit }) {
  ...
  return (
    <div style={{ background: colors.ink, borderRadius: 'var(--radius-xl)', padding: 20, color: '#fff', boxShadow: 'var(--shadow-card)' }}>
      ...
      <div style={{ display: 'flex', flexDirection: stacked ? 'column' : 'row', gap: stacked ? 16 : 26, marginTop: 18, paddingTop: 16, borderTop: '1px solid rgba(255,255,255,0.1)' }}>
        ...
        <button
          ...
          className="a-btn"
          style={{ fontFamily: 'var(--font-sans)', alignSelf: stacked ? 'stretch' : 'center', flexShrink: 0, height: 44, padding: '0 16px', borderRadius: 'var(--radius-md)', background: colors.hi, color: colors.hiInk, border: 'none', fontSize: 13, fontWeight: 700, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 6, whiteSpace: 'nowrap' }}
        >
